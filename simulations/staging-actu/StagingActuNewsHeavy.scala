...
class StagingActuNewsHeavy extends Simulation {
  ...
  val scn = scenario("staging-actu-news-heavy").group("page") {
    exec(http("staging-actu-news-heavy").get("/news/high-school-students-try-out-an-epfl-chemistry-lab/")
  )}
