...
class StagingActuSubscribe extends Simulation {
  ...
  val scn = scenario("staging-actu-channel-subscribe").group("page") {
    exec(http("staging-actu-channel-subscribe").get("/subscription/subscribe/1/")
  )}
