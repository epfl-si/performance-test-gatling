...
class TestActuNewsMedium extends Simulation {
  ...
  val scn = scenario("test-actu-news-medium").group("page") {
    exec(http("test-actu-news-medium").get("/news/des-connexions-se-creent-pour-depasser-la-paralysi/")
  )}
