...
class TestActuNewsHeavy extends Simulation {
  ...
  val scn = scenario("test-actu-news-heavy").group("page") {
    exec(http("test-actu-news-heavy").get("/news/high-school-students-try-out-an-epfl-chemistry-lab/")
  )}
