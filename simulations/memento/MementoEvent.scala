...
class MementoEvent extends Simulation {
  ...
  val scn = scenario("memento-event").group("page") {
    exec(http("memento-event").get("/event/neural-circuits-for-goal-directed-adaptive-motor-c/")
  )}
